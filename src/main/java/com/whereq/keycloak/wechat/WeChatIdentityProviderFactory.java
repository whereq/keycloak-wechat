...
public class WeChatIdentityProviderFactory extends
    ...
    public List<ProviderConfigProperty> getConfigProperties() {
        return ProviderConfigurationBuilder.create()
                ...
                .property().name(WeChatIdentityProvider.CUSTOMIZED_LOGIN_URL_FOR_PC)
                .label("PC 登录 URL")
                .helpText("PC 登录 URL 的登录页面，可以配置为一个自定义的前端登录页面，用来展示公众号带参二维码")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()

                .property().name(WeChatIdentityProvider.OPEN_CLIENT_ID)
                .label("开放平台 Client ID")
                .helpText("当用户使用微信开放平台登录时，要使用的 Client ID，即微信开放平台的 appid")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
                .property().name(WeChatIdentityProvider.OPEN_CLIENT_SECRET)
                ...
                .property().name(WeChatIdentityProvider.WMP_APP_ID)
                .label("小程序 appId")
                .helpText("小程序的 appid")
                .type(ProviderConfigProperty.STRING_TYPE)
                .add()
                .property().name(WeChatIdentityProvider.WMP_APP_SECRET)
                .label("小程序 appSecret")
